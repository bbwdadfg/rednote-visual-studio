...
/**
 * 清理内容，移除话题标签，优化特殊字符列表
 */
function cleanContent(content) {
    return content
        .replace(/#[^#\n]*\[话题\]#/g, '')  // 移除 #xxx[话题]# 格式
        .replace(/#[^#\n]*#(?!\s*\n)/g, '') // 移除单独的 #xxx# 格式（但保留标题）
        // 将特殊字符列表转换为标准列表格式
        .replace(/^(▫️|⚡|💡|📊|🚀|📱|💻|✨|📐|💪|🎯|⭐|🔥|💎|🌟|🎨|🎪|🎭|🎨|🎯|🎪|🎭)\s+(.+)$/gm, '- $1 $2')
        .replace(/\n\s*\n\s*\n/g, '\n\n')   // 清理多余空行
        .trim();
...
function splitContentBySeparator(body) {
    ...
    return parts.map(p => cleanContent(p.trim())).filter(p => p);
}
 ...
 * 智能换行处理
 */
function smartLineBreak(text, maxLength = 12) {
    if (text.length <= maxLength) return text;

    // 尝试在标点符号处换行
    const punctuations = ['！', '？', '。', '，', '、', '：', '；'];
    for (let i = Math.min(maxLength, text.length - 1); i >= maxLength * 0.6; i--) {
        if (punctuations.includes(text[i])) {
            return text.slice(0, i + 1) + '<br>' + smartLineBreak(text.slice(i + 1), maxLength);
        }
    }

    // 如果没有合适的标点符号，在中间位置换行
    const breakPoint = Math.floor(maxLength * 0.8);
    return text.slice(0, breakPoint) + '<br>' + smartLineBreak(text.slice(breakPoint), maxLength);
}
