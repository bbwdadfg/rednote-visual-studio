...
4. AI美化：集成Nano Banana Pro图片美化功能
5. 文案优化：智能优化小红书文案内容
...
示例:
  ...
  python render_xhs_v2.py note.md --enhance --enhance-style illustration
  python render_xhs_v2.py note.md --optimize-copy --enhance
  python render_xhs_v2.py --list-styles
    ...
    if args.optimize_copy:
        ...
        try:
            ...
            print(f"\n📋 标题候选:")
            for i, title in enumerate(optimized_result['titles'][:3], 1):
                print(f"  {i}. {title}")
...
"""
            ...
            # 保存优化后的文件
            optimized_file = str(Path(args.markdown_file).parent / f"{Path(args.markdown_file).stem}_optimized.md")
            ...
            # 使用优化后的文件进行渲染
            args.markdown_file = optimized_file
    ...
    # 渲染基础图片
    generated_images = asyncio.run(render_markdown_to_cards(args.markdown_file, args.output_dir, args.style))
    ...
    if args.enhance:
        ...
        try:
            ...
            enhanced_images = enhancer.enhance_multiple_images(
                ...
                intensity=args.enhance_intensity,
